    ...
    def subscribe(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            if Subscriptions.objects.filter(
                    following=interest_user, user=request.user
            ).exists():
                return Response(
            ...
            Subscriptions.objects.create(following=interest_user,
                                         user=request.user)
            serializer = SubscriptionsSerializer(
                interest_user,
                context={"request": request},
...
def post_delete_relationship_user_with_object(request, pk, model, message):
    ...
    if request.method == "POST":
        if model.objects.filter(recipe=recipe, user=request.user).exists():
            return Response(
                {"errors": f"Рецепт с номером {pk} "
                           f"уже у Вас в {message}."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    obj_recipe = model.objects.filter(recipe=recipe, user=request.user)
    if obj_recipe:
        obj_recipe.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
    return Response(
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def favorite(self, request, pk=None):
        return post_delete_relationship_user_with_object(
            request=request, pk=pk, model=FavoriteRecipeUser,
            message="избранном"
        )
    ...
    def shopping_cart(self, request, pk=None):
        return post_delete_relationship_user_with_object(
            request=request, pk=pk, model=ShoppingCartUser,
